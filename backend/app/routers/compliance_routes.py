...
# -----------------------
# NEW: Audit Trail PDF Report (Admin Only)
# -----------------------
@router.get("/audit-trail/report")
def audit_trail_report(request: Request, current_user=Depends(get_current_user)):
    """
    Generate a PDF report of all audit logs (admin actions).
    ...
    try:
        ...
        logs = list(audit_logs_collection.find().sort("timestamp", -1).limit(500))
        ...
        elements = []
        from datetime import datetime
        title = Paragraph("<b>Audit Trail Report</b>", styles["Title"])
        subtitle = Paragraph("Complete Admin Activity Log", styles["Normal"])
        gen = Paragraph(f"Generated: {datetime.utcnow().isoformat()} UTC", styles["Normal"])
        admin_info = Paragraph(f"Generated by: <b>{current_user.get('email', 'Unknown Admin')}</b>", styles["Normal"])
        total_logs = Paragraph(f"Total Records: <b>{len(logs)}</b>", styles["Normal"])
        elements.extend([title, subtitle, gen, admin_info, total_logs, Spacer(1, 20)])
        
        # Table headers
        table_data = [["#", "Timestamp", "Admin Email", "User Email", "Doc ID", "Action", "Decision", "Notes"]]
        ...
        for idx, log in enumerate(logs, 1):
            ...
            admin_email = str(log.get("adminEmail", "") or log.get("admin", "") or "")
            user_email = str(log.get("userEmail", "") or log.get("user", "") or "")
            doc_id_raw = log.get("docId", "") or log.get("documentId", "")
            doc_id = str(doc_id_raw) if doc_id_raw else ""
            doc_id_display = doc_id[-8:] if len(doc_id) > 8 else doc_id  # Show last 8 chars
            
            action_raw = log.get("action", "")
            action = str(action_raw) if action_raw else ""
            
            decision_raw = log.get("decision", "")
            decision = str(decision_raw) if decision_raw else ""
            
            notes_raw = log.get("notes", "") or log.get("reason", "")
            notes = str(notes_raw) if notes_raw else ""
            
            # Truncate notes safely
            notes_display = notes[:100] + "..." if len(notes) > 100 else notes
            
            row = [
                str(idx),
                Paragraph(timestamp, small),
                Paragraph(admin_email, small),
                Paragraph(user_email, small),
                Paragraph(f"...{doc_id_display}" if doc_id else "", small),
                Paragraph(action.upper() if action else "", small),
                Paragraph(decision.upper() if decision else "", small),
                Paragraph(notes_display, small),
            ]
            table_data.append(row)
        
        # If no logs, add a message row
        if len(logs) == 0:
            table_data.append(["", Paragraph("No audit logs found", small), "", "", "", "", "", ""])
        
        col_widths = [0.35 * inch, 1.2 * inch, 1.5 * inch, 1.5 * inch, 0.9 * inch, 0.7 * inch, 0.7 * inch, 2.0 * inch]
        table = Table(table_data, colWidths=col_widths, repeatRows=1)
        table.setStyle(TableStyle([
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
        ...
        summary_text = f"Total Actions: {len(logs)}<br/>"
        for action, count in action_counts.items():
            summary_text += f"• {action.capitalize()}: {count}<br/>"
        summary_text += f"<br/>Decisions:<br/>"
        for decision, count in decision_counts.items():
            if decision and decision != "unknown":
                summary_text += f"• {decision.capitalize()}: {count}<br/>"
        ...
        filename = f"audit_trail_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.pdf"
